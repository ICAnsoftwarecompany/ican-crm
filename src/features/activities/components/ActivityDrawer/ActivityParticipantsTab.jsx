export function ActivityParticipantsTab({ activity }) {
  ...
  if (!participants.length) {
    return <p className="rounded-lg border border-dashed border-[var(--border)] p-4 text-sm font-semibold text-[var(--text-muted)]">لا يوجد مشاركون مسجلون.</p>
  }
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      {participants.map((participant) => (
        <article key={participant.id || participant.user_id || participant.email} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-3">
          ...
          <span className="mt-2 inline-flex rounded-full bg-slate-50 px-2 py-1 text-[11px] font-black text-slate-600">{participant.status || participant.pivot?.status || 'invited'}</span>
        </article>
      ))}
    </div>
  )
