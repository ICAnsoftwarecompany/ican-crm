...
## عرض الأتمتة باستخدام VisualFlow

تستخدم صفحة `/automation` الآن `WorkflowVisualCanvas` داخل `WorkflowBuilder` لعرض شجرة الـ workflow عبر المحرك العام `shared/components/visual-flow`. يحوّل `utils/workflowVisualGraph.js` الـ trigger والخطوات والتفرعات ونهايات المسارات الفارغة إلى عقد وحواف للعرض فقط. نقاط `+` تحت نهاية كل مسار تفتح مكتبة الخطوات نفسها، والضغط على عقدة موجودة يفتح لوحة الخصائص الحالية. تظل الشجرة في `WorkflowBuilder` هي مصدر الحقيقة للحفظ والتحقق؛ الرسم لا يكتب عقدًا أو حواف جديدة إلى نموذج البيانات، ولا يسمح بوصلات حرة أو سحب العقد لتغيير ترتيب التنفيذ. التكبير والتحريك والخريطة المصغّرة أدوات عرض. لا يوجد backend لتنفيذ الـ workflow، والحفظ ما زال محليًا كما هو موثق أدناه.
