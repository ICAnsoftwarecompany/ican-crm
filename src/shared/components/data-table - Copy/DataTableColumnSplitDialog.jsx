...
export function DataTableColumnSplitDialog({
  ...
  useEffect(() => {
    ...
    const configuredIds = Array.isArray(value?.rightColumnIds)
      ? value.rightColumnIds.filter((columnId) => dataColumns.some((column) => column.id === columnId))
      : []

    setRightIds(configuredIds.length ? configuredIds : getDefaultRightIds(dataColumns))
  ...
  const leftIds = dataColumns.filter((column) => !rightSet.has(column.id)).map((column) => column.id)
  const leftSet = new Set(leftIds)
  const canSave = rightIds.length > 0 && leftIds.length > 0

  const moveToRight = (columnId) => {
    setRightIds((current) => (current.includes(columnId) ? current : [...current, columnId]))
  }

  const moveToLeft = (columnId) => {
    setRightIds((current) => {
  ...
  const applyAutoSplit = () => {
    setRightIds(getDefaultRightIds(dataColumns))
  }

  const handleSave = () => {
    if (!canSave) return
    onSave?.({
      rightColumnIds: rightIds.filter((columnId) => dataColumns.some((column) => column.id === columnId)),
    })
  }

  const renderColumnChoice = (column, target) => {
    const isRight = rightSet.has(column.id)
    const isActive = target === 'right' ? isRight : leftSet.has(column.id)
    const onClick = target === 'right' ? () => moveToRight(column.id) : () => moveToLeft(column.id)
  ...
  return createPortal(
    <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex max-h-[min(720px,calc(100vh-2rem))] w-[min(900px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h3 className="truncate text-base font-black text-slate-900">تخصيص تقسيم الأعمدة</h3>
              <p className="mt-1 text-xs font-semibold text-slate-500">
                اختر الأعمدة التي تظهر في جدول اليمين، وباقي الأعمدة تظهر في جدول اليسار.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50/60 p-5">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <div className="text-xs font-bold text-slate-500">
              جدول اليمين: {rightIds.length} عمود / جدول اليسار: {leftIds.length} عمود
            </div>
            <button
              type="button"
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <section className="min-w-0 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-3 flex items-center justify-between gap-2">
                <h4 className="text-sm font-black text-slate-900">جدول اليمين</h4>
                <span className="rounded-full bg-[#E8F9FA] px-2 py-1 text-[11px] font-black text-[#007A80]">
                  {rightIds.length}
                </span>
              </div>
              <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
                {dataColumns.map((column) => renderColumnChoice(column, 'right'))}
              </div>
            </section>

            <section className="min-w-0 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-3 flex items-center justify-between gap-2">
                <h4 className="text-sm font-black text-slate-900">جدول اليسار</h4>
                <span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">
                  {leftIds.length}
                </span>
              </div>
              <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
                {dataColumns.map((column) => renderColumnChoice(column, 'left'))}
              </div>
            </section>
          </div>
          ...
          {!canSave && (
            <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700">
              يجب اختيار عمود واحد على الأقل في اليمين وعمود واحد في اليسار.
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 bg-white px-5 py-4">
          ...
          >
            {enabled ? 'إيقاف تقسيم الأعمدة' : 'إلغاء'}
          </button>
