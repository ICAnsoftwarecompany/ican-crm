...
export function OpportunityDrawerHeader({ opportunity }) {
  ...
  const statusMeta = getOpportunityStatusMeta(opportunity.status, t)
  const priorityMeta = getOpportunityPriorityMeta(opportunity.priority, t)
    ...
    <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-sm text-[var(--text-muted)]">
            {getOpportunityTypeLabel(opportunity.type, t)} · {opportunity.product?.name}
          </p>
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs text-[var(--text-muted)]">{t('opportunities.scoreOutOf100')}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-[var(--text-muted)]">{t('opportunities.estimatedValue')}</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-[var(--text)]">{opportunity.assigned_user?.name || t('opportunities.unassigned')}</p>
