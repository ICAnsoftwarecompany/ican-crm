...
function describeTimelineMeta(event, t) {
  if (!event.meta) return null

  if (event.type === 'status_changed' && event.meta.to) {
    return t('opportunities.timelineMeta.statusChanged', { status: event.meta.to })
  }
  if (event.type === 'score_changed' && event.meta.to !== undefined) {
    return t('opportunities.timelineMeta.scoreChanged', { from: event.meta.from ?? '-', to: event.meta.to })
  }
  if (event.type === 'assigned' && event.meta.to) {
    return t('opportunities.timelineMeta.assignedTo', { name: event.meta.to })
  }
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="font-bold text-[var(--text)]">{t('opportunities.activityTimeline')}</h4>
      </div>

      {timeline.length === 0 ? (
        <p className="text-sm text-[var(--text-muted)]">{t('opportunities.noActivity')}</p>
      ) : (
        <ol className="relative border-s border-[var(--border)] ps-4 grid gap-4">
          {timeline.map((event) => {
            const Icon = EVENT_ICONS[event.type] || CircleDot
            const description = describeTimelineMeta(event, t)
            ...
            return (
              <li key={event.id} className="relative">
                ...
                <p className="text-sm font-bold text-[var(--text)]">{event.label || getTimelineEventLabel(event.type, t)}</p>
                {description && <p className="text-xs text-[var(--text-muted)] mt-0.5">{description}</p>}
                <p className="text-[11px] text-[var(--text-light)] mt-1">
                  {event.actor?.name ? `${event.actor.name} · ` : ''}{formatRelativeTime(event.at, Date.now(), i18n.language, t)}
                </p>
              </li>
            )
          })}
